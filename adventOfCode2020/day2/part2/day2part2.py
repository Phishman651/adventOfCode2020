...
def checkPositions(pos1,pos2,charLookup,string):
    print(charLookup)
    print("Pos1: " + str(pos1) + ": " + string[pos1])
    print("Pos2: " + str(pos2) + ": " + string[pos2])
    if((string[pos1] == charLookup) and (string[pos2] == charLookup)):
        return False
    elif((string[pos1] == charLookup) or (string[pos2] == charLookup)):
        return True
    else:
        return False
...
for i in range(listLength):
    ...
    #adjust for no concept of index zero
    minOccur = minOccur - 1
    maxOccur = maxOccur - 1
    print("Adj. minOccur = " + str(minOccur))
    print("Adj. maxOccur = " + str(maxOccur))

    # if((numOccurrences >= minOccur) and (numOccurrences <= maxOccur)):
    if(checkPositions(minOccur,maxOccur,charOccur,remainingString) == True):
